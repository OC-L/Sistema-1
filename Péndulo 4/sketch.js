...
function initializeAudio() {
  ...
  let reverb = new Tone.Reverb({
    wet: .1, // half dry, half wet mix
    decay: 1 // decay time in seconds
  });
  
  const chorus = new Tone.Chorus(1, 2.5, 2).toDestination().start();
  ...
  scale = Tonal.Scale.get("E5" + flavour).notes;
  scale = scale.concat(Tonal.Scale.get("F#5 " + flavour).notes);
  scale = scale.concat(Tonal.Scale.get("C6 " + flavour).notes);
