...
function initializeAudio() {
  ...
  const chorus = new Tone.Chorus(25, 2.5, 2).toDestination().start();
  
  const tremolo = new Tone.Tremolo(1, 0.55).toDestination().start();
  ...
  scale = Tonal.Scale.get("A2 " + flavour).notes;
  scale = scale.concat(Tonal.Scale.get("F2" + flavour).notes);
  scale = scale.concat(Tonal.Scale.get("F#4 " + flavour).notes);
