...
function initializeAudio() {
  ...
  //const chorus = new Tone.Chorus(5, 2.5, 2).toDestination().start();
  
  const tremolo = new Tone.Tremolo(0.5, 0.55).toDestination().start();
  
  mixer.connect(reverb);
  reverb.connect(tremolo);
  //chorus.connect(tremolo);
  tremolo.toDestination();

  let flavour = "minor";
  scale = Tonal.Scale.get("F8 " + flavour).notes;
  scale = scale.concat(Tonal.Scale.get("B5" + flavour).notes);
  scale = scale.concat(Tonal.Scale.get("F#7" + flavour).notes);
