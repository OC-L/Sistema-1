...
function initializeAudio() {
  ...
  const chorus = new Tone.Chorus(0.5, 2.5, 2).toDestination().start();
  
  const tremolo = new Tone.Tremolo(25, 0.55).toDestination().start();
  ...
  let flavour = "harmonic minor";
  scale = Tonal.Scale.get("F6 " + flavour).notes;
  scale = scale.concat(Tonal.Scale.get("B5" + flavour).notes);
  scale = scale.concat(Tonal.Scale.get("F#6 " + flavour).notes);
  
  Tonal.Collection.shuffle(scale);

  for (let i = 0; i < scale.length; i++) {
    pendulums[i] = new Pendulum(0.99 + i * (1 / 60), scale[i]);
  }
}
